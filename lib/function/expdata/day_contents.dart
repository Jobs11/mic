const List<String> arcane = [
  "소멸의여로",
  "츄츄아일랜드",
  "레헬른",
  "아르카나",
  "모라스",
  "에스페라",
  "문브릿지",
  "고통의미궁",
  "리멘",
];

const List<String> grandis = [
  "세르니움",
  "호텔아르크스",
  "오디움",
  "도원경",
  "아르테리아",
  "카르시온",
  "탈라하트",
];

const List<String> arcaneimg = [
  "assets/images/areas/journey.png", // 소멸의여로
  "assets/images/areas/chuchu.png", // 츄츄아일랜드
  "assets/images/areas/lachelein.png", // 레헬른
  "assets/images/areas/arcana.png", // 아르카나
  "assets/images/areas/morass.png", // 모라스
  "assets/images/areas/esfera.png", // 에스페라
  "assets/images/areas/moonbridge.png", // 문브릿지
  "assets/images/areas/labyrinth.png", // 고통의미궁
  "assets/images/areas/limen.png", // 리멘
];

const List<String> grandisimg = [
  "assets/images/areas/cernium.png", // 세르니움
  "assets/images/areas/arcs.png", // 호텔아르크스
  "assets/images/areas/odium.png", // 오디움
  "assets/images/areas/shangri.png", // 도원경
  "assets/images/areas/arteria.png", // 아르테리아
  "assets/images/areas/carcion.png", // 카르시온
  "assets/images/areas/tallahart.png", // 탈라하트
];

const Map<String, int> daylevellimit = {
  "소멸의여로": 200,
  "츄츄아일랜드": 210,
  "레헬른": 220,
  "아르카나": 225,
  "모라스": 230,
  "에스페라": 235,
  "문브릿지": 245,
  "고통의미궁": 250,
  "리멘": 255,

  "세르니움": 260,
  "호텔아르크스": 265,
  "오디움": 270,
  "도원경": 275,
  "아르테리아": 280,
  "카르시온": 285,
  "탈라하트": 290,
};

Map<String, bool> dayquest = {
  "소멸의여로": false,
  "츄츄아일랜드": false,
  "레헬른": false,
  "아르카나": false,
  "모라스": false,
  "에스페라": false,
  "문브릿지": false,
  "고통의미궁": false,
  "리멘": false,

  "세르니움": false,
  "호텔아르크스": false,
  "오디움": false,
  "도원경": false,
  "아르테리아": false,
  "카르시온": false,
  "탈라하트": false,
};

Map<String, dynamic> monsterpark = {
  "지역": "소멸의여로",
  "입장여부": false,
  "이미지": arcaneimg[0],
};
